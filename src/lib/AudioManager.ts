...
export class AudioManagerService {
  ...
  public initUserGestureHandler(): void {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        ...
        if (this.coffeePlaying) {
          ...
          if (
            this.coffeeAudioCtx &&
            this.coffeeAudioCtx.state === 'running'
          ) {
        ...
        if (this.coffeeModalOpen && this.coffeePlaying) {
          if (
            this.coffeeAudioCtx &&
            this.coffeeAudioCtx.state === 'suspended'
          ) {
          ...
          !this.coffeeModalOpen &&
          this.bgUserWantsPlaying
        ) {
          if (
            this.bgAudioCtx &&
            this.bgAudioCtx.state === 'suspended'
          ) {
    ...
    const handleFirstGesture = () => {
      ...
      // If background music wants to play but isn't actually playing
      // in audio element/synth
      if (
        !this.coffeeModalOpen &&
        this.bgUserWantsPlaying
      ) {
        if (
          !this.bgPlayer.isPlaying &&
          this.bgOscNodes.length === 0
        ) {
          this.playBgMusic();
        } else if (
          this.bgPlayer.hasAudio() &&
          !this.bgPlayer.isPlaying
        ) {
  ...
  public openCoffeeModal(): void {
    ...
    // Automatically pause background music
    // without resetting bgUserWantsPlaying
    this.pauseBgMusic();
  ...
  public closeCoffeeModal(): void {
    ...
    // If background music was playing before opening
    // the modal AND user didn't disable it manually, resume it
    if (
      this.bgWasPlayingBeforeModal &&
      this.bgUserWantsPlaying
    ) {
      this.playBgMusic();
    }
  ...
  public playCoffeeTrack(index: number): void {
    ...
    const track =
      COFFEE_TRACKS[index] || COFFEE_TRACKS[0];
      ...
      if (this.coffeePlayer.hasAudio()) {
        ...
        this.coffeePlayer.resume().then((success) => {
          if (success) {
            ...
            this.playCoffeeTrack(
              this.coffeeTrackIndex
            );
          }
  ...
  public toggleCoffeeMute(): void {
    ...
    if (
      ...
      this.coffeeGainNode.gain.setValueAtTime(
        this.coffeeMuted ? 0 : 0.12,
        this.coffeeAudioCtx.currentTime
      );
    }
  ...
  private playCoffeeSynthFallback(
    track: CoffeeTrackInfo
  ): void {
    this.stopCoffeeSynth();
  ...
  private stopCoffeeSynth(): void {
    ...
    if (
      this.coffeeAudioCtx &&
      this.coffeeAudioCtx.state !== 'closed'
    ) {
...
export const audioManager =
  AudioManagerService.getInstance();
